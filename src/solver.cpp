...
)
{
    ...
    int pathColor = board.getCell(start.row, start.col).color;
    set<Position> region = allowedRegion;
    region.insert(start);
    region.insert(goal);
    vector<Position> basePath = bfsShortestPath(board, start, goal, region, pathColor);
    if (basePath.empty())
        return result;
...
bool Solver::solve(Board& board)
{
    ...
    vector<Position> fallbackPath;  
    int bestScore = INT_MAX;
    int fallbackScore = INT_MAX;
    for (auto& candidate : chosen.paths)
    {
        ...
        bool passes = forwardCheck(temp, finishedColors, chosen.color);
        if (passes)
        {
            if (score < bestScore ||
        ...
        else
        {
            if (score < fallbackScore ||
                (score == fallbackScore && (fallbackPath.empty() || candidate.size() < fallbackPath.size())))
            {
                fallbackScore = score;
